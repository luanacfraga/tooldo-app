...
export function useExecutorDashboard(input: {
  ...
  return useQuery({
    queryKey: executorDashboardKeys.byCompanyAndPeriod(input.companyId, input.dateFrom, input.dateTo),
    queryFn: () => companiesApi.getExecutorDashboard(input.companyId, { dateFrom: input.dateFrom, dateTo: input.dateTo }),
